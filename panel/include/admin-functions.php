<?php
	...
	class AdminFunctions extends Database {
		...
		function addBanner($data,$file) {
			...
			$query = "insert into ".PREFIX."banner_master (banner_img,link,active,display_order,created) values ('$image_name','$link', '$active', '$display_order','$date')";
		...
		function updateBanner($data,$file) {
			...
			$query = "update ".PREFIX."banner_master set banner_img = '".$image_name."', active='$active', link='$link', display_order='$display_order' where id='$id'";
			return $this->query($query);
		}
		...
		function updateCategory($data,$file) {
			...
			if(isset($file['image_name']['name']) && !empty($file['image_name']['name'])) {
				...
				$image_name = $SaveImage->uploadCroppedImageFileFromForm($file['image_name'], 222, $cropData, $imgDir, $file_name.'-'.time().'-1');
		...
		function updateWelcomeContent($data){
			...
			$allowTags = "<strong><em><b><p><u><ul><li><ol><s><sub><sup><h1><img><h2><h3><h4><h5><h6><div><i><span><br><table><tr><th><td><thead><tbody><a>";
			$description = $this->escape_string($this->strip_selected($data['description'],$allowTags));
			$qry = $this->query("SELECT * FROM ".PREFIX."gallery_content");
			if($this->num_rows($qry) > 0){
			$query = "update ".PREFIX."gallery_content set description='".$description."' where id='".$id."'";
			}else{
				$query = "insert into ".PREFIX."gallery_content (description) values('".$description."')";
			}
			$this->query($query);
		}
		...
		//New Functions
		//Vehicle Details Funcitons
		function getUniqueVehicleDetails($id){
			$id = $this->escape_string($this->strip_all($id));
		...
		function addVehicleDetails($data){
			...
			$owner_email = $this->escape_string($this->strip_all($data['owner_email']));
			$owner_number = $this->escape_string($this->strip_all($data['owner_number']));
			$driver_name = $this->escape_string($this->strip_all($data['driver_name']));
			$driver_email = $this->escape_string($this->strip_all($data['driver_email']));
			$driver_number = $this->escape_string($this->strip_all($data['driver_number']));
			$vehicle_number = $this->escape_string($this->strip_all($data['vehicle_number']));
			$vehicle_code = $this->escape_string($this->strip_all($data['vehicle_code']));
			$active = $this->escape_string($this->strip_all($data['active']));

			$query = "INSERT INTO ".PREFIX."vehicle_master (owner_name, owner_email, owner_mobile, driver_name, driver_email, driver_mobile, vehicle_number, vehicle_code, active) value ('".$owner_name."', '".$owner_email."', '".$owner_number."', '".$driver_name."', '".$driver_email."', '".$driver_number."', '".$vehicle_number."', '".$vehicle_code."', '".$active."')";
		...
		function updateVehicleDetails($data){
			...
			$query = "UPDATE ".PREFIX."vehicle_master SET owner_name = '".$owner_name."', owner_email = '".$owner_email."', owner_mobile = '".$owner_number."', driver_name = '".$driver_name."', driver_email = '".$driver_email."', driver_mobile = '".$driver_number."', vehicle_number = '".$vehicle_number."', vehicle_code = '".$vehicle_code."', active = '".$active."' WHERE id = '".$id."'";
		...
		function deleteVehicleDetail($id){
			...
			$query = "delete from ".PREFIX."vehicle_master where id='".$id."'";
			$this->query($query);
			return true;
		}
		...
		function getUniqueVideoDetails($id){
			$id = $this->escape_string($this->strip_all($id));
			$query = "SELECT * FROM ".PREFIX."video_master WHERE id = '".$id."' ";
			return $this->fetch($this->query($query));
		}

		function addVideoDetails($data, $file){
			$display_order = $this->escape_string($this->strip_all($data['display_order']));
			$active = $this->escape_string($this->strip_all($data['active']));
			$allowedExts = array("mp4");
			$extension = pathinfo($file['video']['name'], PATHINFO_EXTENSION);
			$video = '';
					
			move_uploaded_file($file["video"]["tmp_name"],"video/" . $file["video"]["name"]);
			$video = $file["video"]["name"];
			$query = "INSERT INTO ".PREFIX."video_master (video, display_order, active) VALUES ('".$video."', '".$display_order."', '".$active."')";
			$this->query($query);

		}

		function updateVideoDetails($data, $file){
			$id = $this->escape_string($this->strip_all($data['id']));
			$display_order = $this->escape_string($this->strip_all($data['display_order']));
			$active = $this->escape_string($this->strip_all($data['active']));

			$video = $file["video"]["name"];
			$videodetails = $this->getUniqueVideoDetails($id);
			if(!empty($video)){
				move_uploaded_file($file["video"]["tmp_name"],"video/" . $file["video"]["name"]);
			}else{
			...
			$query = "UPDATE  ".PREFIX."video_master SET video = '".$video."', display_order = '".$display_order."', active = '".$active."' ";
			$this->query($query);
			
		}

		function deleteVideoDetail($id){
			$id = $this->escape_string($this->strip_all($id));
			$query = "DELETE FROM ".PREFIX."video_master where id = '".$id."' ";
			$this->query($query);
			return true;
		}
	} 
